...
function nebula_resources() {
  wp_enqueue_style( 'css', get_template_directory_uri() . '/assets/app.css' );
  wp_enqueue_script( 'jQuery', get_template_directory_uri() . '/js/vendors/jquery-1.12.1.min.js', array(), '1.12.1', true );
  wp_enqueue_script( 'modernizer', get_template_directory_uri() . '/js/vendors/modernizer.js', array(), '', true );
...
function ajax_post() {
  ...
  echo '<div class="project-slider-container">';

  $images = get_field('images');
  $text = get_field('project_description');

    if( $images ):
      echo '<div class="project-slider__button project-slider__prev"></div>';
      echo '<div class="project-slider__button project-slider__next"></div>';
      echo '<div class="project-slider">';
        if ($text) :
          echo '<div class="project-slider__slide-text">';
            echo $text;
          echo '</div>';
        endif;

        foreach( $images as $image ):
          echo '<div class="project-slider__slide">';
            echo '<img src="' . $image['url'] . '" alt="' . $image['alt'] . '" />';
          echo '</div>';
        endforeach;
      echo '</div>';

      echo '<div class="project-slider__info">';
        echo '<h2 class="project-slider__title">';
          the_title();
        echo '</h2>';
        echo '<div class="project-slider__count">';
          echo '<span class="project-slider__current"></span> / <span class="project-slider__total"></span>';
        echo '</div>';
      echo '</div>';
      endif;
    echo '</div>';
